...
#
# IMPORTANT:
# This is intentionally the plain URL value.
# Do not put Markdown links inside a PowerShell script.
...
try {
    ...
    Write-Host ''
    Write-Host `
        'ERROR: Could not create verification report directory.' `
        -ForegroundColor Red
...
function Get-FileContentSafe {
    ...
    try {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            "Unable to read '$Path'. Error: $($_.Exception.Message)"
...
Write-Host `
    '# AWS HYBRID IaC LAB - COMPLETE VERIFICATION                          #' `
    -ForegroundColor Cyan
...
$CloudFormationDirectoryExists = Test-Path `
    -LiteralPath $CloudFormationDirectoryFull `
    -PathType Container

$WorkflowDirectoryExists = Test-Path `
    -LiteralPath $WorkflowDirectoryFull `
    -PathType Container
...
$MainYamlPath = Join-Path `
    $CloudFormationDirectoryFull `
    'main.yaml'

$CloudFormationTfPath = Join-Path `
    $TerraformDirectoryFull `
    'cloudformation.tf'

$VariablesTfPath = Join-Path `
    $TerraformDirectoryFull `
    'variables.tf'

$MainDeployPath = Join-Path `
    $WorkflowDirectoryFull `
    'main-deploy.yaml'

$TerraformWorkflowPath = Join-Path `
    $WorkflowDirectoryFull `
    'terraform.yml'

$DockerWorkflowPath = Join-Path `
    $WorkflowDirectoryFull `
    'docker.yml'

$KubernetesWorkflowPath = Join-Path `
    $WorkflowDirectoryFull `
    'kubernetes.yml'

$EcrYamlPath = Join-Path `
    $NestedDirectoryFull `
    'ecr.yaml'

$EcsYamlPath = Join-Path `
    $NestedDirectoryFull `
    'ecs.yaml'
...
if (Test-CommandExists 'aws') {
    ...
    if ($CallerIdentityResult.ExitCode -eq 0) {
        ...
        try {
            ...
            $CallerIdentity = $CallerIdentityResult.Output |
                ConvertFrom-Json
            ...
            Add-Result `
                ...
                -Status 'FAIL' `
                -Details `
                'AWS returned an unexpected caller identity response.'
        }
    }
    else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Cannot check AWS identity because AWS CLI is unavailable.'
}
...
if ([string]::IsNullOrWhiteSpace($AwsRegionEnvironment)) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        "AWS_REGION is not set locally. Expected region: $ExpectedAwsRegion"
}
elseif ($AwsRegionEnvironment -eq $ExpectedAwsRegion) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        "AWS_REGION is '$AwsRegionEnvironment'; expected '$ExpectedAwsRegion'"
}
...
if (-not [string]::IsNullOrWhiteSpace($AWSAccountId)) {
    ...
    if ($OidcResult.ExitCode -eq 0) {
        ...
        try {
            ...
            $OidcProvider = $OidcResult.Output |
                ConvertFrom-Json
...
if ($null -ne $OidcProvider) {
    ...
    if ($OidcProvider.Url -eq $ExpectedOidcUrl) {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            "Found '$($OidcProvider.Url)' but expected '$ExpectedOidcUrl'"
    }
    ...
    if ($AudienceFound) {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            "Expected audience '$ExpectedOidcAudience' was not found."
    }
}
...
if (Test-CommandExists 'aws') {
    ...
    Add-Result `
        -Category 'IAM' `
        -Check 'GitHub Actions IAM role lookup' `
        -Status 'WARNING' `
        -Details 'AWS CLI is unavailable.'
}
...
if ($null -ne $Role) {
    ...
    try {
        ...
        $TrustDocument = $Role.Role.AssumeRolePolicyDocument

        if ($TrustDocument -is [string]) {

            $TrustPolicyDocument = Convert-IamDocumentToJson `
                -Document $TrustDocument
        }
        else {

            $TrustPolicyDocument = $TrustDocument
        }

        if ($null -ne $TrustPolicyDocument) {

            $TrustJson = Get-PolicySearchJson `
                -PolicyDocument $TrustPolicyDocument
        }
    }
    catch {
    ...
    if ([string]::IsNullOrWhiteSpace($TrustJson)) {
        ...
        try {
            ...
            $TrustJson = $Role.Role.AssumeRolePolicyDocument |
                ConvertTo-Json -Depth 50
        }
        catch {
    ...
    if ($TrustJson -match 'AssumeRoleWithWebIdentity') {
        ...
        Add-Result `
            ...
            -Status 'PASS' `
            -Details `
            'Trust policy allows GitHub OIDC web identity federation.'
    }
    else {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            'Trust policy does not contain AssumeRoleWithWebIdentity.'
    }
    ...
    if (
        $TrustJson -match `
        [regex]::Escape('token.actions.githubusercontent.com')
    ) {
    ...
    if (
        $TrustJson -match `
        [regex]::Escape($ExpectedOidcAudience)
    ) {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            'sts.amazonaws.com was not found in the trust policy.'
    }
    ...
    if (
        $TrustJson -match `
        [regex]::Escape($ExpectedGitHubRepository)
    ) {
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            "Repository '$ExpectedGitHubRepository' was not found."
    }
    ...
    if (
        ...
        Add-Result `
            ...
            -Status 'WARNING' `
            -Details `
            "Could not confirm exact subject restriction: $ExpectedSubject"
    }
}
...
if (Test-CommandExists 'aws') {
    ...
    $PolicyResult = Invoke-SafeCommand `
        -FilePath 'aws' `
        -Arguments @(
            'iam'
            'list-policies'
    ...
    if ($PolicyResult.ExitCode -eq 0) {
        ...
        try {
            ...
            $LocalPolicies = $PolicyResult.Output |
                ConvertFrom-Json
            ...
            if ($MatchingPolicies.Count -gt 0) {
                ...
                Add-Result `
                    ...
                    -Status 'PASS' `
                    -Details `
                    "Policy: $ExpectedPolicyName | ARN: $PolicyArn"
            }
            else {
                ...
                Add-Result `
                    ...
                    -Status 'FAIL' `
                    -Details `
                    "Policy '$ExpectedPolicyName' was not found."
            }
        }
        catch {
...
if ($null -ne $Role) {
    ...
    if ($AttachedPolicyResult.ExitCode -eq 0) {
        ...
        try {
            ...
            $AttachedPolicies = $AttachedPolicyResult.Output |
                ConvertFrom-Json
            ...
            if ($PolicyAttached.Count -gt 0) {
                ...
                Add-Result `
                    ...
                    -Status 'PASS' `
                    -Details `
                    "Policy is attached to $ExpectedRoleName"
            }
            else {
                ...
                Add-Result `
                    ...
                    -Status 'FAIL' `
                    -Details `
                    "Policy '$ExpectedPolicyName' is not attached to the role."
            }
        }
        catch {
...
if (-not [string]::IsNullOrWhiteSpace($PolicyArn)) {
    ...
    if ($PolicyVersionResult.ExitCode -eq 0) {
        ...
        $PolicyVersion = $PolicyVersionResult.Output.Trim()
    ...
    if (-not [string]::IsNullOrWhiteSpace($PolicyVersion)) {
        ...
        if ($PolicyDocumentResult.ExitCode -eq 0) {
            ...
            $PolicyDocument = Convert-IamDocumentToJson `
                -Document $PolicyDocumentResult.Output

            if ($null -ne $PolicyDocument) {

                $PolicyJson = Get-PolicySearchJson `
                    -PolicyDocument $PolicyDocument
                ...
                Add-Result `
                    ...
                    -Status 'PASS' `
                    -Details `
                    'IAM policy document was successfully read and parsed.'
            }
            else {
                ...
                Add-Result `
                    ...
                    -Status 'FAIL' `
                    -Details `
                    'IAM policy document could not be parsed.'
            }
        }
        else {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PolicyJson)) {
        ...
        if (
            ...
            Add-Result `
                ...
                -Status 'WARNING' `
                -Details `
                'CloudFormation permissions were not detected.'
        }
...
if ($null -eq $TerraformCommand) {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Terraform is not installed or is not available in PATH.'
}
else {

    $TerraformVersionResult = Invoke-SafeCommand `
        -FilePath 'terraform' `
        -Arguments @('version')
...
if ($TerraformDirectoryExists) {
    ...
    if ($TerraformFiles.Count -gt 0) {
        ...
        Add-Result `
            ...
            -Status 'PASS' `
            -Details `
            "Found $($TerraformFiles.Count) Terraform .tf file(s)."
...
$TerraformRegionPattern1 = 'region\s*=\s*var\.aws_region'
$TerraformRegionPattern2 = 'region\s*=\s*["''][^"'']+["'']'

if (
    ($TerraformText -match $TerraformRegionPattern1) -or
    ($TerraformText -match $TerraformRegionPattern2)
) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not automatically detect Terraform AWS region configuration.'
}
    ...
    try {
        ...
        if ($FmtResult.ExitCode -eq 0) {
            ...
            Add-Result `
                ...
                -Status 'FAIL' `
                -Details `
                "Formatting check failed. Output: $($FmtResult.Output)"
...
elseif ($null -eq $TerraformCommand) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Terraform is unavailable. Formatting check was skipped.'
}
...
if (
    ...
    try {
        ...
        # initializing or contacting the configured remote backend.
        #
        # This keeps verification focused on Terraform configuration.

        $InitResult = Invoke-SafeCommand `
            -FilePath 'terraform' `
            -Arguments @(
                'init'
                '-backend=false'
        ...
        if ($InitResult.ExitCode -eq 0) {
            ...
            Add-Result `
                ...
                -Status 'PASS' `
                -Details `
                'Terraform initialization completed without the remote backend.'
        }
        else {
            ...
            Add-Result `
                ...
                -Status 'FAIL' `
                -Details `
                "Terraform initialization failed. Output: $($InitResult.Output)"
        }
    }
    finally {
...
elseif ($null -eq $TerraformCommand) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Terraform is unavailable. Initialization was skipped.'
}
...
if (
    ...
    try {
        ...
        $ValidateResult = Invoke-SafeCommand `
            -FilePath 'terraform' `
            -Arguments @(
                'validate'
            )
        ...
        if ($ValidateResult.ExitCode -eq 0) {
            ...
            Add-Result `
                ...
                -Status 'FAIL' `
                -Details `
                "Terraform validation failed. Output: $($ValidateResult.Output)"
        }
    }
    finally {
...
elseif ($null -eq $TerraformCommand) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Terraform is unavailable. Validation was skipped.'
}
...
if ($EcrImageReferences.Count -eq 0) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'No ecr_image_uri references were found in Terraform files.'
}
else {
...
if ($VariablesTf -match 'variable\s+"ecr_image_uri"') {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'variables.tf still declares ecr_image_uri. Review whether it is still required.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'variables.tf does not declare ecr_image_uri.'
}
...
Test-NotContainsText `
    -Content $MainYaml `
    -Text 'EcrImageUri:' `
    -CheckName 'main.yaml must not require EcrImageUri' `
    -Category 'MAIN.YAML' |
    Out-Null
...
if ($MainYaml -match '(?m)^\s*ECSStack\s*:') {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'An ECSStack resource definition exists in main.yaml. ECS should be deployed separately.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'No ECSStack resource definition was detected in main.yaml.'
}
...
Test-NotContainsText `
    -Content $MainYaml `
    -Text 'ECSStackId:' `
    -CheckName 'main.yaml must not expose ECSStackId' `
    -Category 'MAIN.YAML' |
    Out-Null
...
Test-ContainsText `
    -Content $MainYaml `
    -Text 'ECRStack:' `
    -CheckName 'ECR nested stack' `
    -Category 'MAIN.YAML' |
    Out-Null

Test-ContainsText `
    -Content $MainYaml `
    -Text 'ECRRepositoryUri:' `
    -CheckName 'ECR repository URI output' `
    -Category 'MAIN.YAML' |
    Out-Null
...
Test-NotContainsText `
    -Content $CloudFormationTf `
    -Text 'EcrImageUri = var.ecr_image_uri' `
    -CheckName 'Terraform must not pass EcrImageUri during bootstrap' `
    -Category 'TERRAFORM' |
    Out-Null

Test-ContainsText `
    -Content $CloudFormationTf `
    -Text 'ProjectName = var.project_name' `
    -CheckName 'ProjectName parameter' `
    -Category 'TERRAFORM' |
    Out-Null
...
Test-ContainsText `
    ...
    -CheckName 'Environment parameter' `
    -Category 'TERRAFORM' |
    Out-Null

Test-ContainsText `
    -Content $CloudFormationTf `
    -Text 'AmiId = var.ami_id' `
    -CheckName 'AmiId parameter' `
    -Category 'TERRAFORM' |
    Out-Null

Test-ContainsText `
    -Content $CloudFormationTf `
    -Text 'DatabaseUsername = var.database_username' `
    -CheckName 'DatabaseUsername parameter' `
    -Category 'TERRAFORM' |
    Out-Null

Test-ContainsText `
    -Content $CloudFormationTf `
    -Text 'DatabasePassword = var.database_password' `
    -CheckName 'DatabasePassword parameter' `
    -Category 'TERRAFORM' |
    Out-Null
...
if ($UsesGitHubSha) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'Docker workflow uses Git commit SHA information for image tagging.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm github.sha or GITHUB_SHA image tagging.'
}
...
if (
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'Docker workflow contains an ECR login operation.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm an Amazon ECR login step.'
}
...
Test-ContainsText `
    -Content $DockerWorkflow `
    -Text 'docker push' `
    -CheckName 'Docker image push' `
    -Category 'DOCKER' |
    Out-Null
...
if ($EcrYaml -match 'IMMUTABLE') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'ECR repository is configured as IMMUTABLE.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm IMMUTABLE configuration in ecr.yaml.'
}
...
$LatestPushPattern = 'docker\s+push[\s\S]{0,500}:latest'

if ($DockerWorkflow -match $LatestPushPattern) {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'docker.yml appears to push :latest while ECR is immutable.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'No obvious docker push of :latest was detected.'
}
...
if ($EcsYaml -match 'EcrImageUri:') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'ecs.yaml accepts EcrImageUri as an application deployment input.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm EcrImageUri in ecs.yaml.'
}

$EcsUsesImageUri = (
    ($EcsYaml -match 'Image:\s*!Ref\s+EcrImageUri') -or
    ($EcsYaml -match 'Image:\s*\r?\n\s*Ref:\s*EcrImageUri') -or
    ($EcsYaml -match 'Image:\s*.*EcrImageUri')
)
...
if ($EcsUsesImageUri) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'ECS task definition appears to use EcrImageUri.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm that ECS uses EcrImageUri for the container image.'
}
...
if ($MainDeploy -match 'terraform') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'main-deploy.yaml references Terraform.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Terraform workflow reference was not detected.'
}
...
if ($MainDeploy -match 'docker') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'main-deploy.yaml references Docker.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Docker workflow reference was not detected.'
}
...
if ($MainDeploy -match 'kubernetes') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'main-deploy.yaml references Kubernetes.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Kubernetes workflow reference was not detected.'
}
...
$DockerDependsOnTerraform = (
    (
        $MainDeploy -match `
        '(?ms)^\s*docker\s*:.*?^\s*needs\s*:\s*terraform\b'
    ) -or
    (
        $MainDeploy -match `
        '(?ms)^\s*docker\s*:.*?^\s*needs\s*:\s*\[.*terraform.*\]'
...
if ($DockerDependsOnTerraform) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'Docker job depends on Terraform.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm explicit Docker -> Terraform dependency.'
}

$KubernetesDependsOnDocker = (
    (
        $MainDeploy -match `
        '(?ms)^\s*kubernetes\s*:.*?^\s*needs\s*:\s*docker\b'
    ) -or
    (
        $MainDeploy -match `
        '(?ms)^\s*kubernetes\s*:.*?^\s*needs\s*:\s*\[.*docker.*\]'
...
if ($KubernetesDependsOnDocker) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'Kubernetes job depends on Docker.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'Could not confirm explicit Kubernetes -> Docker dependency.'
}
...
if ($TerraformWorkflow -match 'workflow_call') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'terraform.yml supports workflow_call.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'terraform.yml does not contain workflow_call.'
}

if ($TerraformWorkflow -match 'id-token:\s*write') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'terraform.yml contains id-token: write.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'terraform.yml is missing id-token: write.'
}

$AwsRegionWorkflowPattern = `
    'aws-region:\s*\$\{\{\s*vars\.AWS_REGION\s*\}\}'

if ($TerraformWorkflow -match $AwsRegionWorkflowPattern) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'terraform.yml uses vars.AWS_REGION.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'terraform.yml does not use vars.AWS_REGION for aws-region.'
}

$AwsRoleArnWorkflowPattern = `
    'role-to-assume:\s*\$\{\{\s*secrets\.AWS_ROLE_ARN\s*\}\}'

if ($TerraformWorkflow -match $AwsRoleArnWorkflowPattern) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'terraform.yml uses secrets.AWS_ROLE_ARN.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'terraform.yml does not use secrets.AWS_ROLE_ARN.'
}

if ($TerraformWorkflow -match 'aws-actions/configure-aws-credentials@v4') {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'terraform.yml uses configure-aws-credentials@v4.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'configure-aws-credentials@v4 was not detected.'
}
...
if (
    ...
    if ($CloudFormationTemplates.Count -gt 0) {
        ...
        Add-Result `
            ...
            -Status 'PASS' `
            -Details `
            "Found $($CloudFormationTemplates.Count) YAML template(s)."
        ...
        Add-Result `
            ...
            -Status 'FAIL' `
            -Details `
            'No .yaml CloudFormation templates were found.'
    }
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        "Directory does not exist: $CloudFormationDirectoryFull"
}
...
if ($null -eq $AwsCommand) {
    ...
    Add-Result `
        ...
        -Status 'WARNING' `
        -Details `
        'AWS CLI unavailable. CloudFormation validation was skipped.'
}
else {
    ...
    foreach ($Template in $CloudFormationTemplates) {
        ...
        # AWS CLI file URI
        #
        # This performs template validation only.
        # It does not deploy the template.
        # --------------------------------------------------------------

        $TemplateUri = "file://$($Template.FullName)"

        $CloudFormationValidation = Invoke-SafeCommand `
            -FilePath 'aws' `
            -Arguments @(
                'cloudformation'
                'validate-template'
        ...
        if ($CloudFormationValidation.ExitCode -eq 0) {
            ...
            Add-Result `
                ...
                -Status 'PASS' `
                -Details `
                "$($Template.Name) passed AWS CloudFormation validation."
        }
        else {
            ...
            Add-Result `
                ...
                -Status 'FAIL' `
                -Details `
                "$($Template.Name) failed validation. Output: $($CloudFormationValidation.Output)"
        }
    }
}
...
$EcrOutputPattern = `
    'GetAtt\s+ECRStack\.Outputs\.RepositoryUri'

if (
    ($MainYaml -match 'ECRStack:') -and
    ($MainYaml -match 'ECRRepositoryUri:') -and
    ($MainYaml -match $EcrOutputPattern)
) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'main.yaml exposes ECR repository URI from ECRStack.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Could not confirm ECR repository URI output flow.'
}
...
if ($MainYaml -match '(?m)^\s*ECSStack\s*:') {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'main.yaml contains an ECSStack resource definition.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'main.yaml does not create ECS during infrastructure bootstrap.'
}
    ...
    foreach ($Pattern in $PossibleSecretPatterns) {
        ...
        if ($Content -match $Pattern) {
            ...
            Add-Result `
                ...
                -Status 'WARNING' `
                -Details `
                "Potential credential pattern detected in $File. Review manually."
        }
    }
}
...
if (-not $SecretFinding) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'No obvious hard-coded credential patterns were detected.'
}
...
if ($BootstrapCorrect) {
    ...
    Add-Result `
        ...
        -Status 'PASS' `
        -Details `
        'Bootstrap does not require EcrImageUri and does not create ECS.'
}
else {
    ...
    Add-Result `
        ...
        -Status 'FAIL' `
        -Details `
        'Bootstrap still contains an ECR image dependency or ECS resource.'
}
...
Write-Host ''
Write-Host `
    'The following values should exist in GitHub:' `
    -ForegroundColor Yellow
...
Write-Host `
    'GitHub Actions secrets and variables cannot be read directly from AWS CLI.' `
    -ForegroundColor DarkYellow

Write-Host `
    'Verify AWS_REGION and AWS_ROLE_ARN manually in GitHub Settings.' `
    -ForegroundColor DarkYellow
...
if ($TotalFailed -eq 0) {
    ...
    if ($TotalWarnings -eq 0) {
        ...
        $FinalResult = 'PASS - ALL VERIFICATION CHECKS PASSED'
    }
    else {
    ...
    $FinalResult = 'FAIL - VERIFICATION FAILED'
}

# ======================================================================
# SECTION 30 - FINAL VERIFICATION SUMMARY
# ======================================================================
...
if ($WarningResults.Count -eq 0) {
    ...
    foreach ($WarningResult in $WarningResults) {
